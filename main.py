...
working_days_per_year = 52 * days_per_week #calculated by taking the amount of weeks in a year multiplied by the workdays in a week
hours_per_day = hours_per_week / days_per_week #calculated by devideing the hours per week by the days per week
unadjusted_salary = salary_per_hour * hours_per_day * working_days_per_year #calculated by taking the hourly rate multiplied by hours per day multiplied by total work days in a year
adjusted_salary = salary_per_hour * hours_per_day * (working_days_per_year - holidays_per_year - vacation_days_per_year) #calculated the same as unadjusted, just minus the holidays from the work days per year
